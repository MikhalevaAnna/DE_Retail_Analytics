...
├── 📄 init-db.sql                  # Инициализация таблиц и MV ClickHouse
...
**Airflow** управляет всем пайплайном обработки данных.

#### 📋 Задачи DAG

```
┌──────────────────────────────────────────────────────────────────────┐
│                    pipeline_retail_data DAG                          │
│                  Schedule: 0 10 * * * (ежедневно в 10:00)            │
└──────────────────────────────────────────────────────────────────────┘

┌─────────────┐     ┌─────────────┐
│ check_mongo │     │ check_kafka │  ← Проверка соединений
└──────┬──────┘     └──────┬──────┘
         │                 │
         └────────┬────────┘
                  ▼
         ┌────────────────┐
         │ load_mongo_task│ ← Загрузка JSON в MongoDB
         └───────┬────────┘
                 ▼
         ┌────────────────┐
         │check_mongo_data│ ← Проверка загруженных данных в MondoDB
         └───────┬────────┘
                 ▼
         ┌───────────────────────┐
         │transfer_mongo_to_kafka│ ← Публикация в 4 топика
         └───────┬───────────────┘
                 ▼
         ┌─────────────────┐
         │ check_clickhouse│ ← Проверка подключения ClickHouse
         └───────┬─────────┘
                 ▼
         ┌────────────────────────────┐
...
12. Снова переходим к веб-интерфейсу по прямой ссылке `Airflow` [http://localhost:8080](http://localhost:8080) или в корне проекта запустим  [http://ui_dashboard.html](http://ui_dashboard.html) и выберем `Открыть Airflow Webserver`. Запускаем **DAG**  [dags/pipeline_retail_data.py](dags/pipeline_retail_data.py).
13. В веб-интерфейсе  [http://ui_dashboard.html](http://ui_dashboard.html) можно зайти в **Grafana**, посмотреть количество записей по каждому блоку данных.
