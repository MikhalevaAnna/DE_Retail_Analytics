...
7) Создаются таблицы в `ClickHouse` в сыром слое сразу с правильными типами данных.
8) Незаполненные значения сразу обрабатываются, чтобы в **RAW** таблицах не было **NULL** значений.
9) Только для поля `items` - Список товаров с количеством, КБЖУ , производителем, тип данных оставим `String` для сохранения исходных данных.</br>
В этом поле хранится список вложенных **JSON**-структур. 
