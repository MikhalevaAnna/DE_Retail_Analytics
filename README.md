...
└──────┬──────┘     └──────┬──────┘
         ...
         └───────┬────────┘
                 ▼
         ┌────────────────┐
         │check_mongo_data│ ← Валидация данных
         └───────┬────────┘
                 ▼
         ┌─────────────────────┐
         ...
         └───────┬─────────────┘
                 ▼
         ┌─────────────────┐
         │ check_clickhouse│ ← Проверка подключения
         └───────┬─────────┘
                 ▼
         ┌────────────────────────────┐
         │transfer_kafka_to_clickhouse│ ← Consumer в RAW слой, одновременно формируется MART слой
         └───────┬────────────────────┘
                 ▼
         ┌─────────────────┐
         │  spark_etl_task │ ← Считается 30 признаков в Спарке / Запись CSV-файла с результатами в S3
         └─────────────────┘
```
