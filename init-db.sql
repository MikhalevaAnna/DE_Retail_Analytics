-- Создаем несколько баз для Clickhouse
CREATE DATABASE IF NOT EXISTS raw_data;
CREATE DATABASE IF NOT EXISTS mart_data;

-- Создание в сыром слое таблицы с продуктами raw_data.raw_products
...
-- Создание в сыром слое таблицы с клиентами raw_data.raw_customers
...
-- Создание в сыром слое таблицы с магазинами raw_data.raw_stores
...
-- Создание в сыром слое таблицы с покупками raw_data.raw_purchases
...
-- 1. Справочник категорий продуктов mart_data.dim_categories
...
-- MV для dim_categories
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_categories TO mart_data.dim_categories AS
SELECT
    cityHash64(
        lowerUTF8(
            trim(
                replaceRegexpOne(group, '^\\p{So}+\\s*', '')  -- убирает эмодзи в начале
            )
        )
    ) AS category_id,
...
-- 2. Справочник единиц измерения mart_data.dim_units
...
-- MV для dim_units
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_units TO mart_data.dim_units AS
SELECT
...
-- 3. Справочник стран mart_data.dim_countries
...
-- MV для  mart_data.dim_countries
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_countries TO mart_data.dim_countries AS
SELECT
    cityHash64(lowerUTF8(origin_country)) as country_id,
...
UNION ALL
SELECT
    cityHash64(lowerUTF8(location_country)) as country_id,
...
UNION ALL
SELECT
    cityHash64(lowerUTF8(delivery_address_country)) as country_id,
    lowerUTF8(delivery_address_country) as country_name,
...
-- 3. Справочник языков mart_data.dim_language
...
-- MV для mart_data.dim_language
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_language TO mart_data.dim_language AS
SELECT
cityHash64(
...
-- 4. Справочник городов mart_data.dim_cities
...
-- MV для dim_cities (из stores)
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_cities TO mart_data.dim_cities AS
SELECT
...
-- 5. Справочник локаций (адресов) mart_data.dim_address
...
-- MV для dim_locations (из raw_stores и raw_customers)
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_address TO mart_data.dim_address AS
    SELECT
        cityHash64(
    ...
    SELECT
        ...
        if(delivery_address_country != '',
           ...
           0) as country_id,
        cityHash64(lowerUTF8(delivery_address_city)) as city_id,
        lowerUTF8(delivery_address_street) as street,
        lowerUTF8(delivery_address_house) as house,
        lowerUTF8(delivery_address_postal_code) as postal_code,
        0.0 as latitude,
        0.0 as longitude,
        'raw_customers' as record_source,
        now() as load_date,
...
-- 6. Справочник адресов доставки (с квартирой/офисом) mart_data.dim_delivery_addresses
...
-- MV для dim_delivery_addresses (из customers)
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_delivery_addresses TO mart_data.dim_delivery_addresses AS
SELECT
    cityHash64(
        lowerUTF8(delivery_address_city),
        lowerUTF8(delivery_address_street),
...
-- 7. Справочник менеджеров mart_data.dim_managers
...
-- MV для dim_managers (из stores)
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_managers TO mart_data.dim_managers AS
SELECT
    cityHash64(
...
-- 8. Справочник сетей магазинов mart_data.dim_store_networks
...
-- MV для dim_store_networks
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_store_networks TO mart_data.dim_store_networks AS
SELECT
    cityHash64(lowerUTF8(store_network)) as store_network_id,
...
-- 9. Справочник часов работы mart_data.dim_opening_hours
...
-- MV для dim_opening_hours
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_opening_hours TO mart_data.dim_opening_hours AS
SELECT
    cityHash64(
    ...
    coalesce(lowerUTF8(opening_hours_mon_fri), '') as mon_fri,
...
-- 10. Справочник полов mart_data.dim_gender
...
-- MV для dim_gender
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_gender TO mart_data.dim_gender AS
SELECT
...
-- 11. Справочник предприятий mart_data.dim_manufacturer
...
-- MV для dim_manufacturer
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_manufacturer TO mart_data.dim_manufacturer AS
select
    cityHash64(
        lowerUTF8(manufacturer_name),
        lowerUTF8(manufacturer_inn)
    ) as manufacturer_id,
    if(manufacturer_country='', 0, cityHash64(lowerUTF8(manufacturer_country))) as country_id,
    lowerUTF8(manufacturer_name) as manufacturer_name,
    coalesce(lowerUTF8(manufacturer_website), '') as manufacturer_website,
...
--12 Справочник платежных методов mart_data.dim_payment_method
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_payment_method TO mart_data.dim_payment_method AS
SELECT
    multiIf(
...
--13 Справочников типов магазинов mart_data.dim_type_store
...
-- MV для mart_data.dim_type_store
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_dim_type_store TO mart_data.dim_type_store AS
SELECT
    cityHash64(
        lowerUTF8(type)
    ) as type_id,
...
-- Данные о продуктах  mart_data.products_details
...
--- MV для mart_data.products_details
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_products_details TO mart_data.products_details AS
SELECT
    cityHash64(
        lowerUTF8(id)
    ) as product_pk,
    ...
    if(description != '', lowerUTF8(description), '') as description,
    ...
    price as price,
    cityHash64(lowerUTF8(unit)) as unit_id,
    if(origin_country !='', cityHash64(lowerUTF8(origin_country)), 0) as country_id,
...
-- Данные о клиентах mart_data.customers_details
CREATE TABLE IF NOT EXISTS mart_data.customers_details (
    ...
    loyalty_card_number String,
    language_id UInt64,
    payment_method_id Enum8('card' = 1, 'cash' = 2, 'online' = 3, 'other' = 0),
    receive_promotions Bool,
    load_date DateTime,
    record_source String,
    version UInt64 DEFAULT toUnixTimestamp(load_date)
...
-- MV для mart_data.customers_details
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_customers_details TO mart_data.customers_details AS
SELECT
...
-- Данные о магазинах mart_data.stores_details
...
-- MV для mart_data.stores_details
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_stores_details TO mart_data.stores_details AS
SELECT
    cityHash64(
        lowerUTF8(store_id)
    ) as store_pk,
    ...
    if(type !='', cityHash64(lowerUTF8(type)), 0) as type_id,
    if(store_network !='', cityHash64(lowerUTF8(store_network)), 0) as store_network_id,
    arrayMap(x ->
             cityHash64(
                 lowerUTF8(
                     trim(
...
-- Данные о связи покупателей с магазинами mart_data.link_customers_stores
...
-- MV для mart_data.link_customers_stores
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_link_customers_stores TO mart_data.link_customers_stores AS
SELECT
    cityHash64(
    ...
    cityHash64(lowerUTF8(customer_id)) as customer_pk,
    cityHash64(lowerUTF8(purchase_location_store_id)) as store_pk,
    cityHash64(
        lowerUTF8(delivery_address_city),
        lowerUTF8(delivery_address_street),
        lowerUTF8(delivery_address_house),
...
-- Общие данные о покупках mart_data.purchases_details
...
-- MV для mart_data.purchases_details
CREATE MATERIALIZED VIEW IF NOT EXISTS mart_data.mv_purchases_details TO mart_data.purchases_details AS
SELECT
    ...
    multiIf(
        delivery_city != '' AND delivery_street != '' AND delivery_house != '' AND delivery_postal_code != ''
    ...
    is_delivery,
    toTimeZone(purchase_datetime, 'UTC') as purchase_datetime,
    now() as load_date,
    toUnixTimestamp(now()) as version
FROM raw_data.raw_purchases
WHERE
...
-- Подробные данные о составе покупок mart_data.purchase_item_details
